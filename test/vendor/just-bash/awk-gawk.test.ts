...
const KNOWN = new Set<string>([
  ...
  "options: -v RS before BEGIN",
  "options: -F then -v FS",
  "options: -v unknown escape",
  "options: -v escaped backslash",
  "options: -v OFS tab",
  "options: -f one file",
  "options: -f two files",
  "options: -f with an operand",
  "options: -- ends options",
  "options: -- then a dash operand",
  "operands: delete ARGV[1] in BEGIN",
  "operands: ARGV[1] rewritten to an assignment",
  "operands: ARGC lowered in BEGIN",
  "operands: ARGC lowered keeps the first file",
  "operands: an empty ARGV entry is skipped",
  "operands: ARGV entry added in BEGIN",
  "operands: dash between two files",
  "operands: a keyword as a name is fatal",
  "operands: a builtin as a name is fatal",
  "operands: an array name is fatal",
  "operands: assignments before, between and after files",
  "operands: FS per file",
  "operands: only an assignment reads stdin",
  "operands: assignment escapes",
  "operands: assignment not seen in BEGIN",
  "operands: RS as an operand",
  "operands: missing file",
  "operands: dash alone reads stdin",
  "operands: FILENAME of stdin",
  "operands: a missing file stops the run",
  "records: chat RS=--- table",
  "records: chat match array extraction",
  "records: RS a single character",
  "records: RS a single character with a leading separator",
  "records: RS a regex metacharacter",
  "records: paragraph mode",
  "records: paragraph mode single trailing newline",
  "records: paragraph mode no trailing newline",
  "records: paragraph mode CRLF blank line is not blank",
  "records: paragraph mode fields split on newline",
  "records: paragraph mode default FS",
  "records: RS regex",
  "records: RS regex CRLF",
  "records: RT under the default RS",
  "records: RS changed in a rule",
  "records: RS changed in a rule carries to the next file",
  "records: plain getline moves NR, FNR and RT",
  "records: getline var moves NR and FNR, not $0",
  "records: getline from a file leaves NR",
  "records: getline var from a file leaves NR",
  "records: command getline leaves NR",
  "records: command getline var leaves NR",
  "records: close and reopen a file under another RS",
  "records: close and rerun a command",
  "records: close truncates an output file on reopen",
  "records: close answers",
  "records: getline in BEGIN reads the operands",
  "records: getline var in BEGIN reads stdin",
  "fields: paragraph fields after sub",
