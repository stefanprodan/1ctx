...
| `src/network/index.ts`, `src/index.ts` | the package exports `validateAllowList` and `matchesAllowListEntry` | a credential's URL prefix is checked and matched by the rules curl's allow-list uses |
