...
| `src/commands/awk/lexer.ts`, `parser2.ts`, `check.ts` | a name followed by `(` with a space between is a call only for a builtin; for any other name it is a concatenation, `x (y)` joining `x` and `y`, and a user function's name used that way, as a variable or as an array is refused before `BEGIN`, exit 1 | `x (1 ? "b" : "c")` was `function 'x' not defined`, a form models write to join strings |
