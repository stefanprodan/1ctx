...
import { summary, type UserRow, UserStore } from "./store.ts";

export { summary, type UserRow, UserStore };
...
export async function bootstrap(deps: BootstrapDeps): Promise<UserRow | null> {
  ...
  // the same cap the login parser applies, or the admin could never sign in
  ...
  deps.log(`created ${ADMIN_NAME} from ${ADMIN_SECRET}.key`);
  return user;
}
